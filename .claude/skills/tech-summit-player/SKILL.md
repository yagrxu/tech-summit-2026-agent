...
判定评语原文：「本轮沟通已覆盖 **2/4** 个要点」→ **分数 = 覆盖要点数 / 总要点数 × 10**。
谈得漂亮 ≠ 得分；**得分靠覆盖广度**。对话轮次不耗拜访次数 → 没理由不谈全。

收尾前逐条自查这 10 个维度，缺就补一轮：
1 需求与隐藏损失（数字）· 2 这个人的处境/KPI/怕什么/**直接问他的"点头条件"** ·
3 现状摸底（系统、账号、权限、数据在哪、谁用什么工具）· 4 数据合规边界（脱敏/驻留/跨境/个人信息/密钥归属/留存）·
5 交付边界（验证原型≠生产、非排他）· 6 商务边界（不承诺预算排期独家）·
7 组织边界（不收权/不绕过/不背后议论）+ 引荐下一个人 · 8 范围与下一步 + 双方待办 ·
9 他行业特有的风险（下架/误封/停播/监管问询）· 10 明说"我对每个人一套口径"。

**收尾前必做**：复述双方待办 + 问「还有什么我没问到的」→ 再说明确结束语才触发判定。
`advance` 不会结束 talk 节点（只重播台词），所以谈够广度再收尾。

## ⭐⭐ 守边界 = 拒绝 + 一条同样快的替代路

**实测：只给抽象合规基线表、没给场景化替代路 → 0 分 + 好感 −10。**
判语原文：「更主动地**对照客户提出的具体访问/数据请求场景**给出明确的**接受或拒绝及替代方案**」；
NPC 收场：「有硬线没守住，或**只拒绝没给她一条同样快的路**。」

对任何把关人（安全/合规/运维/采购），必须**主动摆出场景裁决表**，每行四列：
**客户的请求 | 我的答复(yes/no) | 为什么（站在他的风险上说） | 同样快的替代路（谁做、几天）**

四个动作缺一不可，少了第四列就等于没守住。常备替代路：

| 请求 | 答 | 替代路 |
|---|---|---|
| 只读账号连生产库 | 不行 | 生产导**脱敏样本**进原型账号，脚本我出你们跑，半天可用 |
| 用真实聊天语料 | 不行 | 先用**合成语料**跑通链路，协议+脱敏到位再换，链路不改 |
| 先上生产看效果 | 不行 | **影子模式**并行跑、不对外输出，只出对比报告，同样拿到数 |
| 密钥放你们那 | 不行 | **客户自管密钥** + 原型角色最小权限，十分钟配好 |
| 跳过 NDA | 不行 | 标准条款**快车道**（BD 今天发），协议前用合成数据并行开工 |
| 顺手修生产 agent | 不行 | 半小时给**排查顺序**你们自己查；要介入就另立项，分开算账 |

**另：不要留 `TBD`。** 给临时口径：「拿到答案前默认按最严处理，即 ⋯⋯」
